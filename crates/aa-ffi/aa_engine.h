/* Generated by cbindgen — do not hand-edit.
 * Source: crates/aa-ffi/src/lib.rs. Regenerate via scripts/build-ios.sh
 * (or `cbindgen --config crates/aa-ffi/cbindgen.toml --crate aa-ffi --output crates/aa-ffi/aa_engine.h`). */
 ...
 */
struct AaEngine *aa_engine_create(const char *scene_id);

/**
 * Destroy the engine and release all resources.
 */
void aa_engine_destroy(struct AaEngine *engine);

/**
 * Resize the character grid the scene renders into.
 */
void aa_engine_set_grid(struct AaEngine *engine, uint32_t width, uint32_t height);

/**
 * Set the colour theme by name ("Hacker", "Amber", "Ice", "Ghost").
 * Unknown names are silently ignored.
 */
void aa_engine_set_theme(struct AaEngine *engine, const char *theme_name);

/**
 * Forward a scene-specific setting (id + numeric value).
 */
void aa_engine_apply_setting(struct AaEngine *engine, const char *setting_id, double value);
 ...
 */
const uint8_t *aa_engine_next_frame(struct AaEngine *engine,
                                    double t,
                                    uint32_t *out_width,
                                    uint32_t *out_height);
